...
export default function NearQSPanel({ listings, threshold = DEFAULT_THRESHOLD, onSort }) {
  ...
  return (
    <div className="nqs-panel">
      <div className="nqs-header" onClick={() => setOpen(o => !o)}>
        ...
        <span className="nqs-header-title">
          Near Quicksell Deals
        </span>
        <span className="nqs-header-count">{deals.length} card{deals.length !== 1 ? 's' : ''} within {threshold}% of QS</span>
        <span className="nqs-header-sub">buy-now price is barely above quicksell floor — near risk-free</span>
            ...
            return (
              ...
              >
                ...
                <div className="nqs-card-info">
                  <div className="nqs-card-name">{l.listing_name || item.name}</div>
                  <div className="nqs-card-meta">
                    <span className="rarity-badge" style={{ background: r.badge, color: r.text, fontSize: 9 }}>
